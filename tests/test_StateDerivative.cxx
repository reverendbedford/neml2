...
TEST_CASE("All combinations of valid State objects can be accessed",
          "[StateDerivative]") {
  StateInfo first;
  ...
  SECTION("Scalar/Scalar") {
    test.set<BatchedScalar>("one", "four", 
  ...
  SECTION("SymR2/Scalar") {
    test.set<BatchedSymR2>("two", "four", 
                           BatchedSymR2(torch::ones({bs,6})));
  ...
  SECTION("Scalar/SymR2") {
    test.set<BatchedSymR2>("one", "three", 
                           BatchedSymR2(torch::ones({bs,6})));
  ...
  SECTION("SymR2/SymR2") {
    test.set<BatchedSymSymR4>("two", "three", 
                              BatchedSymSymR4(torch::ones({bs,6,6})));
