...
TEST_CASE("StandardBatchedTensors have the right shapes, "
          "construct with tensor", "[StandardBatchedTensor]") {
  
  // Batch dimension 10, base dimension (3,4)
  StandardBatchedTensor<3, 4> A(torch::zeros({10,3,4}));
...
TEST_CASE("StandardBatchedTensor can't be created with the wrong base "
          "or batch dimensions", "[StandardBatchedTensor]") {
  REQUIRE_THROWS(StandardBatchedTensor<3, 4>(torch::zeros({10,5,4})));
  REQUIRE_THROWS(StandardBatchedTensor<3, 4>(torch::zeros({3,4})));
}
