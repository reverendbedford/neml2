...
TEST_CASE("Can setup, get, and set State with basic objects",
          "[State]") {
  StateInfo s;
  ...
  SECTION("Set and get a particular value") {
    ...
    REQUIRE(torch::sum(test.get<BatchedScalar>("scalar1")).item<double>()
            == Approx(10));
    test.set<BatchedSymR2>("symmetric1", BatchedSymR2(torch::ones({bs,6})));
    REQUIRE(torch::sum(test.get<BatchedSymR2>("symmetric1")).item<double>()
            == Approx(60));
  }
}

TEST_CASE("Operations on a substate affect the parent",
          "[State]") {
  StateInfo secondary;
  ...
  SECTION("Set through the primary, check secondary") {
    test.set<BatchedScalar>("scalar_in_secondary", 
                            BatchedScalar(torch::ones({bs,1})));
