    ...
    const State & state_n, const State & forces_n)
{
  ...
  state_np1.set<BatchedSymR2>(
      "stress", _C.dot(forces_np1.get<BatchedSymR2>("strain")));
}

void
SmallStrainIsotropicLinearElasticModel::update_linearized(
    StateDerivative & tangent, const State & forces_np1, 
    const State & state_n, const State & forces_n)
{
  // Just repeat the elasticity tensor along the batch dimension
  tangent.set<BatchedSymSymR4>("stress", "strain", 
                               torch::repeat_interleave(
                                   _C.reshape({1,6,6}), forces_np1.batch_size(), 0
                                   )
                               );
}
