...
void StateInfo::add_substate(std::string name, const StateInfo & substate)
{
  ...
  for (auto it : substate.item_locations()) {
    ...
    TorchSize item_size = substate.item_offsets()[loc+1] - 
        substate.item_offsets()[loc];
    _item_locations.insert({item_name, nitems()});
    _item_offsets.push_back(size_storage() + item_size);
  }
}

size_t StateInfo::nitems() const
{
  return _item_locations.size();
}

TorchSize StateInfo::size_storage() const
{
  return _item_offsets.back();
}

size_t StateInfo::nsubstates() const
{
  return _substates.size();
}

TorchShape StateInfo::required_shape(TorchSize nbatch) const
{
